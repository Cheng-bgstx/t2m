...
export class TrackingHelper {
   ...
   */
  convertMotionJointPosPolicyToDataset(jointPosRows) {
    if (!jointPosRows || !Array.isArray(jointPosRows)) {
      return jointPosRows;
    }
    if (!this.mapPolicyToDataset || !this.datasetJointNames.length) {
      return jointPosRows.map((row) => Float32Array.from(row));
    }
    return jointPosRows.map((row) => this._mapPolicyJointPosToDataset(row));
  }
}
